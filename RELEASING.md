...
6. When changing registry dependencies, pin them to immutable compatible commits, starting with the dependency-free theme, then text, then badge. GitHub refs are not inherited by transitive dependencies. Validate the public install after updating the pins.
7. After approval, merge and tag v0.1.0. Publish release notes describing the component scope, installation, supported stack, and measurement behavior.
8. Deploy the docs on Railway using the repository root, build command `pnpm build`, and start command `pnpm --filter @eva-cn/docs exec next start --hostname 0.0.0.0 --port $PORT`.
9. Check the deployed pages and `/r/eva-badge.json`. Repeat the public registry consumer check against the release tag. Verify that transitive dependencies resolve to the intended released source.
